...
import { Drawer, Box, Typography, Switch, Button } from '@mui/material';
  ...
  return (
    <>
            ...
            <ul className="navbar-nav me-auto mb-2 mb-lg-0">
              <li className="nav-item">
                <a
                  ...
                  style={{ color: 'white', paddingLeft: '327px', fontSize: '20px' }}
      ...
      <Drawer anchor="right" open={isDrawerOpen} onClose={() => setIsDrawerOpen(false)}>
        <Box p={2} width={['100%', '400px']} textAlign="left" role="presentation">
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
            <Typography variant="h6" component="div">
              ...
              <div style={{ paddingTop: '10px' }}>
                <input
                  className="pomodoro"
                  style={{ width: '30%', borderRadius: '5px', textAlign: 'center' }}
                  min="0"
                  value={pomodoroInput}
                  onChange={(e) => setPomodoroInput(parseInt(e.target.value, 10) || 0)}
                ></input>
                <input
                  style={{ width: '30%', marginLeft: '15px', borderRadius: '5px', textAlign: 'center', marginTop: '10px' }}
                  min="0"
                  value={shortBreakInput}
                  onChange={(e) => setShortBreakInput(parseInt(e.target.value, 10) || 0)}
                ></input>
                <input
                  style={{ width: '30%', marginLeft: '15px', borderRadius: '5px', textAlign: 'center', marginTop: '10px' }}
                  min="0"
                  value={longBreakInput}
                  onChange={(e) => setLongBreakInput(parseInt(e.target.value, 10) || 0)}
                ></input>
              </div>
              <div style={{ paddingTop: '20px' }}>
            ...
            <div style={{ marginTop: 'auto', paddingTop: '20px', paddingLeft: '0' }}>
              <Button variant="outlined" className="change" onClick={handleApply}>
                Apply
              </Button>
            </div>
          </div>
